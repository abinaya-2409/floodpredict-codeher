...
import { ArrowRight, Building2, CheckCircle2, Lock, Mail, MapPin, ShieldCheck, UserRound, X } from 'lucide-react';
...
export const LoginModal: React.FC<Props> = ({ selectedCity, zones, onLoginSuccess, onSignOut, onClose, initialSession }) => {
  ...
  return <div className="auth-overlay" role="dialog" aria-modal="true" aria-label="Sign in to FloodyPredict"><div className="auth-panel">
    ...
    <div className="auth-brand"><span className="brand-mark"><LogoMark className="h-7 w-7" /></span><div><strong>FloodyPredict</strong><p>Flood intelligence platform</p></div></div>
    ...
    <div className="auth-tabs"><button onClick={() => setMode('citizen')} className={mode === 'citizen' ? 'is-active' : ''}><UserRound size={16} /> Resident</button><button onClick={() => setMode('authority')} className={mode === 'authority' ? 'is-active' : ''}><ShieldCheck size={16} /> Authority</button></div>
