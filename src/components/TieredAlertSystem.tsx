  ...
  const generateMessage = () => {
    ...
    if (selectedLanguage === 'ta') {
      if (selectedTier === 'evacuate') {
        return `[Floodylink - அவசர வெளியேற்ற எச்சரிக்கை] ${zoneName} பகுதியில் ${depth}செ.மீ வரை வெள்ள நீர் உயர வாய்ப்புள்ளது. தரைதளத்தில் உள்ளவர்கள் உடனடியாக நிவாரண முகாமுக்கு செல்லவும். உதவிக்கு: 1913 / 1070.`;
      } else if (selectedTier === 'warning') {
        return `[Floodylink - வெள்ள எச்சரிக்கை] ${zoneName} பகுதியில் கனமழை தொடர்வதால் வாகனங்களை மேடான பகுதிக்கு மாற்றவும். மின் இணைப்பை சரிபார்க்கவும்.`;
      }
      return `[Floodylink - கண்காணிப்பு அறிக்கை] ${zoneName} பகுதியில் அடுத்த 6 மணி நேரத்திற்கு மிதமான மழை பெய்ய வாய்ப்புள்ளது. கழிவுநீர் வடிகால்கள் கண்காணிக்கப்படுகின்றன.`;
    }
    ...
    // Default English
    if (selectedTier === 'evacuate') {
      return `[Floodylink TIER-3 EVACUATE] Immediate flash flood risk for ${zoneName}. Est depth: ${depth}cm within 45 mins. Evacuate ground floors to designated community centers. Emergency: 1070 / 1913.`;
    } else if (selectedTier === 'warning') {
      return `[Floodylink TIER-2 WARNING] High flood risk in ${zoneName}. Rainfall exceeding drain discharge capacity. Relocate vehicles to elevated parking decks now.`;
    }
    return `[Floodylink TIER-1 WATCH] Heavy rainfall band approaching ${zoneName}. Municipal desilting teams active. Monitor local alerts.`;
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="px-2.5 py-0.5 rounded-full text-micro font-bold uppercase bg-risk-high/20 text-risk-high-ink border border-risk-high/40">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="px-2.5 py-0.5 rounded-full text-micro font-bold uppercase bg-risk-severe/20 text-risk-severe-ink border border-risk-severe/40">
        ...
        >
          <div className="flex items-center justify-between">
            <span className="px-2.5 py-0.5 rounded-full text-micro font-bold uppercase bg-risk-critical/20 text-risk-critical-ink border border-risk-critical/40 animate-pulse">
        ...
        <div className="lg:col-span-6 space-y-4">
          ...
          <div className="p-4 bg-bg border border-line rounded-card">
            <div className="text-xs font-bold text-muted mb-3 flex items-center justify-between">
              ...
              <span className="text-micro font-mono text-risk-low-ink">● 4G/5G Broadcast Active</span>
            </div>
              ...
              <div className="flex items-center justify-between text-xs">
                <div className="flex items-center space-x-2">
                  <span className="p-1 rounded-control bg-accent/20 text-accent">
                    <AlertTriangle className="w-3.5 h-3.5 text-risk-critical-ink" />
                  </span>
                ...
                <div key={log.id} className="p-2.5 rounded-control bg-surface border border-line text-xs">
                  <div className="flex items-center justify-between">
                    <span className={`px-2 py-0.2 rounded text-micro font-bold uppercase ${
                      log.tier === 'evacuate' ? 'bg-risk-critical/20 text-risk-critical-ink' : 'bg-risk-severe/20 text-risk-severe-ink'
                    }`}>
